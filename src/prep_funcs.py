...
def prep_coefs_fn(coefs_fn):
     # коэффициенты
    coef_df = pd.read_excel(coefs_fn)
    ...
    # тут и дубли по участникам и не далеко от друг друга
...
def prep_fighters_fn(fighters_fn, fighters=[]):
    fighters_stat_df = pd.read_csv(fighters_fn)
    
    fighters_stat_df['full_name'] = fighters_stat_df['First'].str.cat(fighters_stat_df['Last'], sep=' ', na_rep='')

    if len(fighters)!=0:
        fighters_stat_df = fighters_stat_df[fighters_stat_df['full_name'].isin(fighters)]

    # рост в дюймах, чтобы в см - round(x * 2.54, 2)
    fighters_stat_df['height_stat'] = fighters_stat_df['Ht.'].str.extract(r"(\d+)'\s*(\d+)\"").apply(lambda x: int(x[0])*12+int(x[1]) if pd.notna(x[0]) and pd.notna(x[1]) else np.nan, axis=1)
    
    # размах в дюймах
    fighters_stat_df['reach_stat'] = fighters_stat_df['Reach'].str.extract(r"([\d.]+)\"")
    fighters_stat_df['reach_stat'] = fighters_stat_df['reach_stat'].astype('float')

    fighters_stat_df = fighters_stat_df.assign(dob = lambda x: pd.to_datetime(x['dob'], format='%b %d, %Y').dt.to_period(freq='D'))
    
    fighters_stat_df = fighters_stat_df.rename(columns={'Stance':'stance'})[['full_name', 'stance', 'dob', 'height_stat', 'reach_stat']]
    
    return fighters_stat_df
    
# fighters = ['Ilia Topuria', 'Max Holloway']
def prep_strikes_fn(fights_in_fn, fighters=[]):
    
    # --------------------------------
    df = pd.read_csv(fights_in_fn)
    
    if len(fighters)!=0:
        df = df[(df.Fighter_left.isin(fighters)) | (df.Fighter_right.isin(fighters))]

    df = df.drop_duplicates()
    df = df.assign(event_date1=pd.to_datetime(df['Date'], format='%Y-%m-%d', errors='coerce').dt.to_period(freq='D'))\
.assign(event_date2=pd.to_datetime(df['Date'], format='%Y-%m-%d 00:00:00', errors='coerce').dt.to_period(freq='D'))\
...
.drop(columns=['event_date1', 'event_date2'])
    ...
    df_all['ctrl_stat'] = df_all['ctrl_stat'].astype('float')
