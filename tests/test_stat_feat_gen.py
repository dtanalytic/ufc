...
class StatFeatGenTest(unittest.TestCase):
    ...
    def test_rolling_stats(self):
        ...
        self.assertTrue(self.stat_df.query('Fighter=="Conor McGregor" and event_date=="2014-09-27"')['ctrl_stat_exp_max_min_d'].iloc[0].round(2)==0.5)

        
        if self.conf['stat_feat_gen']['rol_window_size']==3:

            self.assertTrue(self.stat_df.query('Fighter=="Conor McGregor" and event_date=="2021-07-10"')['lose_stat_rol_sum'].iloc[0]==2)
    
            self.assertTrue(self.stat_df.query('Fighter=="Conor McGregor" and event_date=="2014-09-27"')['kd_stat_rol_sum'].iloc[0].round(2)==1.14)
